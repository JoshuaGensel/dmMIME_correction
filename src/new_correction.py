...
def get_pool_data(path : str, protein_concentrations : list):
    ...
    ground_truth = np.loadtxt(path + f"target1_{protein_concentrations[0]}_target2_{protein_concentrations[0]}/ground_truth.csv", delimiter=",")
    ...
    for protein_concentration_1 in protein_concentrations:
        for protein_concentration_2 in protein_concentrations:
            ...
            round_1_sequence_effects.append(np.loadtxt(path + f"target1_{protein_concentration_1}_target2_{protein_concentration_2}/round_1/sequence_effects.csv", delimiter=","))
            round_2_sequence_effects.append(np.loadtxt(path + f"target1_{protein_concentration_1}_target2_{protein_concentration_2}/round_2/sequence_effects.csv", delimiter=","))
...
def infer_logK_sequences(initial_frequencies: np.array, selected_frequencies: np.array, total_protein_concentration: float, c: float):

    # compute free protein concentration
    free_protein_concentration = total_protein_concentration - np.sum(selected_frequencies)
...
def logK_inference(path : str, protein_concentrations : list, c : float):
    ...
    for i in range(len(protein_concentrations)):
        ...
        for j in range(len(protein_concentrations)):
            ...
            logK_sequences_r1.append(infer_logK_sequences(round_1_initial_frequencies[i*len(protein_concentrations) + j], round_1_selected_frequencies[i*len(protein_concentrations) + j], protein_concentration_1, c))
            ...
            logK_sequences_r2.append(infer_logK_sequences(round_2_initial_frequencies[i*len(protein_concentrations) + j], round_2_selected_frequencies[i*len(protein_concentrations) + j], protein_concentration_2, c))
    ...
    return logK_sequences_r1, logK_mutations_r1, logK_sequences_r2, logK_mutations_r2, np.log(ground_truth), np.log(round_1_sequence_effects), np.log(round_2_sequence_effects)
